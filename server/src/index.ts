...
const baristaEmails = new Set(
  (process.env.BARISTA_EMAILS ?? "")
    .split(",")
...
app.get("/orders/slot-availability", async (req: Request, res: Response) => {
...
app.post("/auth/barista-signup", async (req: Request, res: Response) => {
...
app.post("/auth/barista-grant", async (req: Request, res: Response) => {
...
async function isBaristaEmail(email: string | null) {
  if (!email) return false;
  if (baristaEmails.has(email)) return true;
...
async function getOptionalAuthContext(req: Request, res: Response) {
  const header = req.headers.authorization;
  const token = header?.startsWith("Bearer ") ? header.slice(7) : null;
...
async function loadProductCategoryMap() {
  ...
  if (error) {
    ...
    return new Map<string, "coffee" | "dessert">();
  }
  return new Map(
    (data ?? []).map((product) => [product.name.toLowerCase(), product.category])
  );
}

function countCoffeeItems(
  orderItems: OrderItem[] | null | undefined,
  totalItems: number | null | undefined,
  productMap: Map<string, "coffee" | "dessert">
) {
  ...
  return orderItems.reduce((sum, item) => {
    ...
    if (category === "dessert") return sum;
...
app.get("/orders", async (req: Request, res: Response) => {
  ...
  const ids =
    typeof rawIds === "string" && rawIds.trim().length
      ? rawIds
          .split(",")
          .map((id) => id.trim())
          .filter(Boolean)
